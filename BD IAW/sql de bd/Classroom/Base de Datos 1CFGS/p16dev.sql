...
select * from etapa
where llegada !=salida and numero=numero-1

rem esto es lo que han comentado en clase q hay q hacer
[
select et1.*
from etap et1, etapa 2
...
select * from ciclista c, maillot m, llevar l
where l.codigo=m.codigo AND l.dorsal=c.dorsal
group by c.dorsal, nombre, edad, equipo
[having count(distinct l.codigo) = (selecr count(codigo) from maillot);]

rem distint para evitar maillots repetidos
rem copiado de clase, a partir de los [] no entiendo

rem 4.- Todas las etapa que tienen 2 puertos de montaña han pasado a tener los mismos kms que
la etapa número 5.
