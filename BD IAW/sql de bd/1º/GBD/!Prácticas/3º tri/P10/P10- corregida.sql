...
WHERE p.DNI = c.DNI_cp AND c.calle = v.calle
...
SELECT dni_cp 
FROM casaparticular c, vivienda vivienda
WHERE v.calle=c.calle
AND v.numero=c.numero
AND UPPER(nombre_zona) = 'CENTRO';
...
SELECT p.nombre_persona, p.DNI
FROM persona p, piso pi
WHERE p.dni=pi.dni_p
WHERE pi.calle LIKE 'Da%' 
AND pi.numero = 20;
...
SELECT nombre_zona, SUM(metros) AS total_metros_cuadrados
FROM viviendas
...
SELECT *
FROM casaparticular c, vivienda v
WHERE c.calle= v.calle
AND c.numero=v.numero
AND c.metros_c < 100 
AND v.nombre_zona = 'SUBURBIO TERMINAL';
---
rem subconsulta:
...
SELECT nombre_zona, calle, numero, escalera, planta, puerta, metro
FROM vivienda
WHERE lower(nombre_zona) = 'centro';
...
SELECT nombre_zona, tipo_vivienda, AVG(metros) AS tamano_promedio
FROM vivienda
GROUP BY nombre_zona, tipo_vivienda
HAVING AVG(metros) > 100;

rem todo lo q está en el select q no sean agrupamientos se pone en el group by


rem 8. Obtener todas las zonas de la ciudad en las que existen casas particulares
rem  (Ayuda: para realizar esta consulta hay que obtener las zonas en la que hay vivienda y en las que no hay bloques )

rem --aqui salen casas particulares:
SELECT nombre_zona
FROM vivienda v, casaparticular C
WHERE v.calle=c.calle 
AND v.numero=c.numero;

rem --aqui los bloques de casa:
SELECT nombre_zona
FROM vivienda v, bloquecasas B
WHERE v.calle=b.calle AND v.numero=b.numero;

rem --aqui subconsulta final:
SELECT DISTINCT nombre_zona
FROM vivienda v, casaparticular C
WHERE v.calle=c.calle 
AND v.numero=c.numero
AND nombre_zona NOT IN
(SELECT nombre_zona
FROM vivienda v, bloquecasas B
WHERE v.calle=b.calle AND v.numero=b.numero);


rem --forma fácil donde usamos tipo_vivienda:

SELECT DISTINCT nombre_zona
FROM vivienda
WHERE tipo_vivienda= 'C' AND nombre_zona NOT IN
(SELECT nombre_zona FROM vivienda WHERE tipo_vivienda='B');



rem 9. Obtener el número de piso que existen en el bloque Felipe II, numero 14

SELECT COUNT(*)
FROM piso
WHERE UPPER(calle) = 'FELIPE II' AND numero = 14;
