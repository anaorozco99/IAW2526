...
SELECT NOMBRE 
FROM PROVEEDORES PR, ENVIOS EN 
WHERE EN.CODIGO_P = PR.CODIGO_P AND CODIGO_PZ = 'P2';
...
SELECT CODIGO_PZ 
FROM ENVIOS 
WHERE CANTIDAD BETWEEN 200 AND 300 GROUP BY CODIGO_PZ;

/*3. Obtener el nombre de las piezas de color rojo suministradas por los proveedores de la
ciudad de Londres.*/

SELECT PZ.NOMBRE 
FROM PIEZA PZ, PROVEEDORES PR, ENVIOS EN 
WHERE EN.CODIGO_P = PR.CODIGO_P AND EN.CODIGO_PZ = PZ.CODIGO_PZ AND COLOR = 'ROJO' AND PR.CIUDAD = 'LONDRES';

/*4. Obtener los datos de los envíos de más de 100 unidades, mostrando también el nombre
del proveedor y el de la pieza.*/

SELECT EN.*, PZ.NOMBRE "NOMBRE PIEZA", PR.NOMBRE "NOMBRE PROV" 
FROM ENVIOS EN, PIEZA PZ, PROVEEDORES PR 
WHERE EN.CODIGO_P = PR.CODIGO_P AND EN.CODIGO_PZ = PZ.CODIGO_PZ AND CANTIDAD > 100;
...
SELECT PR.*, EN.CODIGO_PZ, CANTIDAD, PZ.* 
FROM ENVIOS EN, PIEZA PZ, PROVEEDORES PR 
WHERE EN.CODIGO_P = PR.CODIGO_P AND EN.CODIGO_PZ = PZ.CODIGO_PZ;

/*6. Obtener el nombre de los proveedores que no suministran la pieza con el código P2*/

SELECT DISTINCT(NOMBRE) 
FROM PROVEEDORES PR, ENVIOS EN 
WHERE EN.CODIGO_P = PR.CODIGO_P
MINUS
SELECT DISTINCT(NOMBRE) 
FROM PROVEEDORES PR, ENVIOS EN 
WHERE EN.CODIGO_P = PR.CODIGO_P AND CODIGO_PZ = 'P2';
...
SELECT * 
FROM ENVIOS 
ORDER BY CANTIDAD DESC FETCH FIRST 1 ROW ONLY;

SELECT * 
FROM ENVIOS 
WHERE CANTIDAD = (SELECT MAX(CANTIDAD) FROM ENVIOS);
...
FROM ENVIOS 
GROUP BY CODIGO_P 
HAVING AVG(CANTIDAD) > (SELECT AVG(CANTIDAD) FROM ENVIOS);

/*10. Obtener todas las parejas de nombre de proveedores que sean de la misma ciudad.*/

SELECT P1.NOMBRE, P2.NOMBRE, P1.CIUDAD, P2.CIUDAD 
FROM PROVEEDORES P1, PROVEEDORES P2 
WHERE P1.CIUDAD = P2.CIUDAD AND P1.NOMBRE > P2.NOMBRE;
